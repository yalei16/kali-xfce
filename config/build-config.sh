...
sources_config() {
  if [[ "$1" == *"debian-"* ]]; then
    ...
    echo "KALI_MIRROR=https://mirrors.tuna.tsinghua.edu.cn/kali/"
    echo "KALI_SECURITY_MIRROR=http://http.kali.org/kali/"
  fi
}
...
get_packages() {
  ...
  if [[ "$system_type" == *"debian-"* ]]; then
    ...
    base_packages="$base_packages fonts-noto-cjk kali-archive-keyring"
  fi
    ...
    case "$desktop_env" in
      ...
      "xfce")
        if [[ "$system_type" == *"kali-"* ]]; then
          echo "$base_packages kali-desktop-xfce kali-linux-core"
        else
          echo "$base_packages xfce4 xfce4-goodies lightdm"
        fi
        ;;
      *)
        # 默认返回基础包
