...
case "${KALI_VERSION:-}" in
	kali-rolling|kali-last-snapshot) _force_pw_gnome=true ;;
esac
if [ "$_force_pw_gnome" = true ] && [ "$DESKTOP_ENV" = "gnome" ]; then
	_use_pulseaudio=false
	echo "[$(date +'%Y-%m-%d %H:%M:%S')] [06b]   └─ ${UBUNTU_VERSION:-${DEBIAN_VERSION:-$KALI_VERSION}} GNOME：强制 PipeWire（Remote Login）"
fi
