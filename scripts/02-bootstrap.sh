...
if [[ "$SYSTEM_TYPE" == *"debian-"* ]]; then
    ...
    if [ ! -f "$KALI_KEYRING" ]; then
        echo "[$(date +'%Y-%m-%d %H:%M:%S')] [02] ❌ 错误: 缺少 Kali 签名密钥环 $KALI_KEYRING" >&2
        echo "[$(date +'%Y-%m-%d %H:%M:%S')] [02]    请先安装: apt-get install kali-archive-keyring" >&2
        exit 1
    fi
else
...
if [ "$BOOTSTRAP_TOOL" = "mmdebstrap" ]; then
    ...
    if [[ "$SYSTEM_TYPE" == *"kali-"* ]]; then
        debootstrap --arch=$ARCH --keyring="$KALI_KEYRING" $OS_VERSION rootdir $MIRROR
    else
