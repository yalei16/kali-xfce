...
KALI_TSUNING_MIRROR="${KALI_TSUNING_MIRROR:-https://mirrors.tuna.tsinghua.edu.cn/kali/}"
...
EOF
    ...
    if [ -n "$KALI_VERSION" ]; then
        cat > rootdir/etc/apt/sources.list << EOF
deb $KALI_TSUNING_MIRROR $KALI_VERSION main contrib non-free non-free-firmware
EOF
    fi
fi
