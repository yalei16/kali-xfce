...
if [[ "$SYSTEM_TYPE" == *"debian-"* ]]; then
   BASE_PACKAGES="bash-completion sudo apt-utils ssh openssh-server nano network-manager systemd-boot initramfs-tools chrony curl wget locales tzdata fonts-wqy-microhei dnsmasq iptables iproute2 zram-tools udev dbus kmod ca-certificates wireless-regdb"
	...
	BASE_PACKAGES="bash-completion sudo apt-utils ssh openssh-server nano network-manager net-tools systemd-boot initramfs-tools chrony curl wget locales tzdata fonts-noto-cjk dnsmasq iptables iproute2 zram-tools udev dbus kmod ca-certificates wireless-regdb kali-archive-keyring"
fi
    ...
    case "$DESKTOP_ENV" in
        ...
        "xfce")
            if [[ "$SYSTEM_TYPE" == *"kali-"* ]]; then
                DESKTOP_PACKAGES="kali-desktop-xfce kali-linux-core"
            else
...
if [[ "$SYSTEM_TYPE" == *"debian-"* || "$SYSTEM_TYPE" == *"kali-"* ]]; then
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] [06]   └─ 修复 Debian/Kali dpkg 错误"
...
install_qcom_local_debs() {
	...
	local required=(
		...
		modemmanager-qrtr-sm8150_*_arm64.deb
	)
	...
	local mm_deb
	mm_deb="$(ls -1 "$deb_dir"/modemmanager-qrtr-sm8150_*_arm64.deb 2>/dev/null | sort -V | tail -1)"
	if [ -z "$mm_deb" ]; then
		echo "[$(date +'%Y-%m-%d %H:%M:%S')] [06] ❌ 缺少: $deb_dir/modemmanager-qrtr-sm8150_*_arm64.deb" >&2
...
EOF
        ...
        echo "[$(date +'%Y-%m-%d %H:%M:%S')] [06]   └─ 配置 LightDM 自动登录"
        install -d rootdir/etc/lightdm
        cat > rootdir/etc/lightdm/lightdm.conf << 'EOF'
[Seat:*]
autologin-user=user
autologin-user-timeout=0
EOF
        # XFCE 默认使用 LightDM，确保显示管理器启动
...
elif [[ "$SYSTEM_TYPE" == *"phosh"* || "$SYSTEM_TYPE" == *"gnome"* || "$DESKTOP_ENV" == "xfce" ]]; then
